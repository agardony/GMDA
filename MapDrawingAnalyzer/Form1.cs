...
namespace MapDrawingAnalyzer
{
    public partial class Form1 : Form
    {
        ...
        private void fillParticipantsMapCoordinates() // load coordinates of setch map into participantsMapCoordinates for use in BDR calculations
        {
            string currentMode = Properties.Settings.Default.currentMode;
            ...
            switch (currentMode)
            {
                ...
                case "advanced":
                    double maxR = 0.0;
                    int maximizeR_rotCorrection = 0;
                    // compute configural BDR parameters (to get configural Theta score)

                    for (int i = 0; i < 4; i++) // for each possible rotation correction
                    {
                        // enter rotated coordinates into cart coords
                        enterRotationAdjustedAdvCoords(i);
                    
                        double[] BDR_parameters = computeBDR(actualMapCoordinates, participantsMapCoordinates);
                        double r = BDR_parameters[1];
                        // maximize configural r rotation correction procedure
                        if (r > maxR) 
                        { 
                            maxR = r;
                            maximizeR_rotCorrection = i;
                        }
                    }
                    enterRotationAdjustedAdvCoords(maximizeR_rotCorrection);
                    break;
            }
        }

        private void enterRotationAdjustedAdvCoords(int rot)
        {
            // clear out participant's map coordinates
            if (participantsMapCoordinates != null)
            {
                for (int i = 0; i < participantsMapCoordinates.Length; i++)
                {
        ...
        private void updateCurrentMode(string s) // updates and saves current mode, possible params: "basic", "advanced"
        {
            Properties.Settings.Default.currentMode = s;
            Properties.Settings.Default.Save();
        }

        private void updateBoxBorderColor(PictureBox p) // adv mode: activated when border color pallette is clicked, sets the border color to the tag of the clicked Picturebox p
        {
        ...
        private void updateBoxCenterColor(PictureBox p) // adv mode: activated when center color pallette is clicked, sets the center color to the tag of the clicked Picturebox p
        {
        ...
        private void newCoordinatesFile() // Hides analysis form (Form 1) and displays build coordinates file form
        {
            string currentMode = Properties.Settings.Default.currentMode;
            switch (currentMode)
            {
                case "basic":
                    Form2 bcfb = new Form2(this); // basic coordinates file builder
                    this.Hide();
                    bcfb.Show();
                    break;
                case "advanced":
                    ...
                    this.Hide();
                    acfb.Show(); // adv coordinates file builder
                    break;
            }
        }

        private bool openMapImage() // opens map image for map analysis, error checks iamge to make sure it is square
        {
            string currentMode = Properties.Settings.Default.currentMode;
            currentRot = 0;
            ...
            if (openFile.ShowDialog() == DialogResult.OK)
            {
                ...
                else
                {
                    if (currentMode.Equals("basic"))
                    {
                        ...
                        {
                            highlightPanel(this.panel_highlight_labels); // bring user's attention to landmark labels
                        }
                        else
                        {
        ...
        private void loadConfigOpenDialog(bool multifile) // 1st step to loading configuration, starts open file dialog for loadConfig, calls loadConfiguration(), batch analysis handled within this function
        {
            string currentMode = Properties.Settings.Default.currentMode;
        ...
        private void takeScreenshot() // takes screenshot of program and opens file save dialog
        {
            ...
            if (d_saveImage.ShowDialog() == DialogResult.OK)
            {
                ...
                System.Threading.Thread.Sleep(600); // used to be 300 ms but it was getting caught sometimes
        ...
        private void batchReanalysis() // event for batch analysis click
        {
            bool multifile = true;
            loadConfigOpenDialog(multifile);
        }

        private void reset() // resets program to starting state, initializing variables
        {
        ...
        private void restoreDefaults() // restores program to default variables
        {
        ...
        private void calculate(bool isPreview, bool multifile = false, string fileName = "") // calculate measures, opt params: are multiple files being processed, file name of saved file
        {
                ...
                if (!saveName.Equals(""))
                {
                    if (!multifile)
                    {
                        outfile = new StreamWriter(Application.StartupPath + "//Data//GMDA_" + saveName + "_map_" + cb_coordsFiles.SelectedItem + "_" +
                            string.Format("{0:yyyy-MM-dd_hh-mm-ss-tt}", now) + "_Raw.csv");
                        outfile2 = new StreamWriter(Application.StartupPath + "//Data//GMDA_" + saveName + "_map_" + cb_coordsFiles.SelectedItem + "_" +
                            string.Format("{0:yyyy-MM-dd_hh-mm-ss-tt}", now) + "_Summary.csv");
                    }
                    else
                    {
                        outfile = new StreamWriter(Application.StartupPath + "//Data//GMDA_" + saveName + "_Raw.csv");
                        outfile2 = new StreamWriter(Application.StartupPath + "//Data//GMDA_" + saveName + "_Summary.csv");
                    }
                }
                else { return; }
                // write header
                outfile.WriteLine("Source,Target,Actual(N/S),Actual(E/W),Observed(N/S),Observed(E/W),Actual D,Actual D ratio,Actual Angle,Observed D,Observed D ratio,Observed Angle,O-A Distance Ratio,O-A Angle,ABS(O-A) Distance Ratio, ABS(O-A) Angle");
            }
            ...
            pb_calculate.Value--;
            // Table of Contents of allCombos[8]
            ...
            // index 7 = distance between landmark pair
            ...
            pb_calculate.Value--;
            // all calculations of target environment calculated
            ...
            double[] BDR_parameters = computeBDR(actualMapCoordinates, participantsMapCoordinates);
            int numBoxes = actualMapCoordinates[0].Count / 8;
            double[,] Box_BDR_parameters = new double[numBoxes,11];

            if (currentMode.Equals("advanced"))
            {
                Box_BDR_parameters = getBOX_BDR_parameters(numBoxes);
                BDR_parameters = computeBDR(actualMapCoordinates, participantsMapCoordinates);

            }
            pb_calculate.Value += 17;// 5
            pb_calculate.Value--;
            // 0 = canonical score
            // 1 = total canonical comparisons
            // 2 = Scaling Bias
            // 3 = ABS Scaling Bias
            // 4 = Sin(angle)
            // 5 = Cos(angle)
            // 6 = ABS(angle)
            int numTotalLandmarks = actualMapCoordinates[0].Count;
            List<List<double>[]> GMDA_by_landmarks = new List<List<double>[]>();
            for (int i = 0; i < numTotalLandmarks; i++)
            {
                List<double>[] tempList = new List<double>[7];
                for(int j = 0; j < tempList.Length; j++)
                {
                    tempList[j] = new List<double>();
                }
                GMDA_by_landmarks.Add(tempList);
            }
            ...
            for (int i = 0; i < allCombos[0].Count; i++) // make comparisons, update counters
            {
                ...
                { 
                    numCorrect++;
                    GMDA_by_landmarks[source][0].Add(1.0);
                    GMDA_by_landmarks[target][0].Add(1.0);
                }
                // E/W comparison
                if (tempArray[1].Equals(allCombos[3][i])) 
                { 
                    numCorrect++;
                    GMDA_by_landmarks[source][0].Add(1.0);
                    GMDA_by_landmarks[target][0].Add(1.0);
                }
                // added raw distances to outfile, 2.15.14
                string actualDistance = allCombos[7][i];
                int sourceLandmarkNum = source + 1;
                int targetLandmarkNum = target + 1;
                string observedDistance = "";
                ...
                if (!tempArray[0].Equals("M"))
                {
                    ...
                    GMDA_by_landmarks[source][2].Add(OminusADistance);
                    GMDA_by_landmarks[target][2].Add(OminusADistance);
                    GMDA_by_landmarks[source][3].Add(ABSOminusADistance);
                    GMDA_by_landmarks[target][3].Add(ABSOminusADistance);
                    GMDA_by_landmarks[source][4].Add(sinA);
                    GMDA_by_landmarks[target][4].Add(sinA);
                    GMDA_by_landmarks[source][5].Add(cosA);
                    GMDA_by_landmarks[target][5].Add(cosA);
                    GMDA_by_landmarks[source][6].Add(ABSOminusAAngle);
                    GMDA_by_landmarks[target][6].Add(ABSOminusAAngle);
            ...
            if (!isPreview)
            {
                ...
                outfile2.WriteLine("Measure Type Key:\n");
                outfile2.WriteLine("GMDA_c - Configural GMDA Measures");
                outfile2.WriteLine("BDR_c - Configural BDR parameters");
                outfile2.WriteLine("GMDA_i - GMDA Measures (individual landmarks)");
                if (currentMode.Equals("advanced")) { outfile2.WriteLine("BDR_i - BDR parameters (individual landmarks)"); }
                outfile2.WriteLine("\nConfigural GMDA Measures");
                outfile2.WriteLine("\nMeasure Type,Filename,Measure,Score");
            //    outfile2.WriteLine("GMDA_c," + saveName + "," + "Total Correct," + numCorrect);
                outfile2.WriteLine("GMDA_c," + saveName + "," + "Canonical Organization," + percentCorrect);
                outfile2.WriteLine("GMDA_c," + saveName + "," + "SQRT(Canonical Organization)," + Math.Sqrt(percentCorrect));
                outfile2.WriteLine("GMDA_c," + saveName + "," + "Canonical Accuracy," + percentObservedCorrect);
                ...
                if (BDR_parameters[0] == 0.0) { outfile2.WriteLine("\nUnable to calculate BDR parameters"); }
                else
                {
                    ...
                    for (int i = 0; i < numTotalLandmarks; i++)
                    {
                        double CanAcc, ScalingBias, RotationalBias, DistAcc, AngAcc = 0.0;
                        string landmarkName = "";
                        if (!landmarkMissing(i + 1))
                        {
                            ...
                            { 
                                index = index / 8;
                                jLim = 8; // iterate through 8 miniboxes
                            }
                            landmarkName = landmarkNames[index];
                            ...
                            double ySum = 0.0;
                            double xSum = 0.0;
                            double divisor = numDistAngComparisons * jLim;
                            for (int j = 0; j < jLim; j++) // basic mode: one run, adv: 8 runs
                            {
                                CanAcc += GMDA_by_landmarks[i + j][0].Sum();
                                ScalingBias += GMDA_by_landmarks[i+j][2].Sum();
                                ySum += GMDA_by_landmarks[i+j][4].Sum();
                                xSum += GMDA_by_landmarks[i+j][5].Sum();
                                DistAcc += GMDA_by_landmarks[i + j][3].Sum();
                                AngAcc += GMDA_by_landmarks[i+j][6].Sum();
                            }
                            CanAcc = CanAcc / (divisor*2);
                            ScalingBias = ScalingBias / divisor;
                            RotationalBias = (180 / Math.PI) * Math.Atan2(ySum / divisor, xSum / divisor);
                            DistAcc = 1 - (DistAcc / divisor);
                        ...
                        {
                            int index = i;
                            if (currentMode.Equals("advanced")) { index = index / 8; }
                            landmarkName = landmarkNames[index];
                            if (currentMode.Equals("advanced"))
                            {
                                if (i % 8 == 0) // only print one missing statement (not 8)
                                {
                                    outfile2.WriteLine(landmarkName + " is missing. Unable to calculate GMDA measures");
                                }
                            }
                            else
                            {
                    ...
                    if (currentMode.Equals("advanced"))
                    {
                        ...
                        outfile2.WriteLine("\nBDR Parameters for individual landmarks\n");
                        outfile2.WriteLine("Measure Type,Filename,Landmark,Measure,Score");
                        for (int i = 0; i < landmarkNames.Count; i++)
                        {
                            ...
                            {
                                outfile2.WriteLine(landmarkNames[i] + " is missing. Unable to calculate BDR parameters");
                            }
                    ...
                    for (int j = 0; j < A_prime_B_prime[0].Count; j++)
                    {
                        ...
                        int currentIndex = (int)A_prime_B_prime[0][j] - 1;
            ...
            double yBar = sinSum / numDistanceCorrect;
            double xBar = cosSum / numDistanceCorrect;
            double angleMean = Math.Atan2(yBar, xBar);
            double angleMean_deg = (180.0 / Math.PI) * angleMean;
        ...
        {
            string currentMode = Properties.Settings.Default.currentMode;
            for (int i = 0; i < 2; i++) { allCombos[i].Clear(); }
            switch (currentMode)
            {
                case "basic":
                    for (int i = 0; i < numLandmarks; i++)
        ...
        {
            for (int i = 4; i < 8; i++) { allCombos[i].Clear(); }
        ...
        private double[] computeBDR(List<double>[] actualMapCoordinates, List<string>[] participantsMapCoordinates)
        {
            ...
            if (currentMode.Equals("advanced"))
            {
                // "correct" indices to be 1... instead of 0...
                for (int i = 0; i < actualMap[0].Count; i++)
                {
                    actualMap[0][i] = Math.Round(actualMap[0][i] + 1.0, 0);
                }
            }
            ...
            // convert participant's coordinates to doubles, if advanced mode correct indices of P's map and actual map (1... instead of 0...)
        ...
        private double[,] getBOX_BDR_parameters(int numBoxes)
        {
            ...
            {
                // make sub lists for box coords
                List<double>[] AMC = new List<double>[3];
                List<string>[] PMC = new List<string>[3];
                for (int j = 0; j < 3; j++)
                {
                    AMC[j] = new List<double>();
                    PMC[j] = new List<string>();
                }
                // copy sub coords into sub lists
                for (int j = 0; j < 3; j++)
                {
                    for (int k = counter; k < counter + 8; k++)
                    {
                        AMC[j].Add(actualMapCoordinates[j][k]);
                        PMC[j].Add(participantsMapCoordinates[j][k]);
                    }
                }
                double[] tempBDR = computeBDR(AMC, PMC);
                for (int j = 0; j < 11; j++)
                {
                    Box_BDR_parameters[i, j] = tempBDR[j];
                }
                counter += 8;
            }
            return Box_BDR_parameters;
        }
    
        private List<PointD> advancedRotationAdjustment(List<PointD> coords, int rot) // figure out a good comment for this
        {
        ...
        private void aboutToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Gardony Map Drawing Analyzer (GMDA): v. " + currentVersion + "\n\n" +
                ...
                "\n\n If you would like to obtain source code please email the author, " + 
                "Aaron Gardony, at agardony@gmail.com."
                );
        }        
    }
}

...
namespace MapDrawingAnalyzer
{
    public partial class Form1 : Form
    {
        ...
        private void calculate(bool isPreview, bool multifile = false, string fileName = "") // calculate measures, opt params: are multiple files being processed, file name of saved file
        {
            ...
            double[] Box_BDR_means = new double[10];
            for(int i = 0; i < 10; i++){Box_BDR_means[i] = 0.0;}
            if (currentMode.Equals("advanced"))
            {
                int counter = 0;
                ...
                // average box BDR parameters
                int divisor = 0;
                for (int i = 0; i < numBoxes; i++)
                {         
                    if (Box_BDR_parameters[i,0] == 0.0)
                    {
                        continue;
                    }
                    for (int j = 0; j < 10; j++)
                    {
                        Box_BDR_means[j] += Box_BDR_parameters[i, j+1];
                    }
                    divisor++;
                }
                for (int i = 0; i < 10; i++)
                {
                    Box_BDR_means[i] = Box_BDR_means[i] / divisor;
                }
         ...
         //   for (int i = 0; i < 
            ...
            for (int i = 0; i < allCombos[0].Count; i++) // make comparisons, update counters
            {
                ...
                if (tempArray[0].Equals(allCombos[2][i])) { numCorrect++; }
                if (tempArray[1].Equals(allCombos[3][i])) { numCorrect++; }
            ...
            if (!isPreview)
            {
                outfile2.WriteLine("GMDA Measures\n");
                outfile2.WriteLine("Filename,Measure,Score");
                outfile2.WriteLine(saveName + "," + "Total Correct," + numCorrect);
                outfile2.WriteLine(saveName + "," + "Canonical Organization," + percentCorrect);
                outfile2.WriteLine(saveName + "," + "SQRT(Canonical Organization)," + Math.Sqrt(percentCorrect));
                outfile2.WriteLine(saveName + "," + "Canonical Accuracy," + percentObservedCorrect);
                outfile2.WriteLine(saveName + "," + "Num Landmarks Missing," + numMissing);
                outfile2.WriteLine(saveName + "," + "O-A distance mean," + distanceSum / numDistanceCorrect);
                outfile2.WriteLine(saveName + "," + "O-A angle mean," + angleSum / numDistanceCorrect);
                outfile2.WriteLine(saveName + "," + "O-A distance mean (ABS)," + ABSdistanceSum / numDistanceCorrect);
                outfile2.WriteLine(saveName + "," + "O-A angle mean (ABS)," + ABSangleSum / numDistanceCorrect);
                outfile2.WriteLine(saveName + "," + "Distance Accuracy," + (1 - (ABSdistanceSum / numDistanceCorrect)).ToString());
                outfile2.WriteLine(saveName + "," + "Angle Accuracy," + (1 - ((ABSangleSum / numDistanceCorrect) / 180.0)).ToString());
                outfile2.WriteLine("\nBDR Parameters\n");
                if (BDR_parameters[0] == 0.0) { outfile2.WriteLine("\nUnable to calculate BDR parameters"); }
                else
                {
                    // write BDR parameters
                    outfile2.WriteLine(saveName + "," + "r," + Convert.ToString(BDR_parameters[1]));
                    outfile2.WriteLine(saveName + "," + "alpha 1," + Convert.ToString(BDR_parameters[2]));
                    outfile2.WriteLine(saveName + "," + "alpha 2," + Convert.ToString(BDR_parameters[3]));
                    outfile2.WriteLine(saveName + "," + "beta 1," + Convert.ToString(BDR_parameters[4]));
                    outfile2.WriteLine(saveName + "," + "beta 2," + Convert.ToString(BDR_parameters[5]));
                    outfile2.WriteLine(saveName + "," + "scale," + Convert.ToString(BDR_parameters[6]));
                    outfile2.WriteLine(saveName + "," + "theta," + Convert.ToString(BDR_parameters[7]));
                    outfile2.WriteLine(saveName + "," + "DMax," + Convert.ToString(BDR_parameters[8]));
                    outfile2.WriteLine(saveName + "," + "D," + Convert.ToString(BDR_parameters[9]));
                    outfile2.WriteLine(saveName + "," + "DI," + Convert.ToString(BDR_parameters[10]));
                    if (currentMode.Equals("advanced"))
                    {
                        outfile2.WriteLine("\nBDR Parameters averaged across landmarks\n");
                        outfile2.WriteLine("Filename,Measure,Score");
                        string[] measureNames = { "r", "alpha 1", "alpha 2", "beta 1", "beta 2", "scale", "theta", "DMax", "D", "DI" };
                        for (int i = 0; i < 10; i++)
                        {
                            outfile2.WriteLine(saveName + "," + measureNames[i] + "," + Convert.ToString(Box_BDR_means[i]));

                        }
                        outfile2.WriteLine("\nBDR Parameters for each landmark\n");
                        outfile2.WriteLine("Filename,Landmark,Measure,Score");
                        for (int i = 0; i < landmarkNames.Count; i++)
                        {
                            if (Box_BDR_parameters[i, 0] != 0)
                            {
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "r," + Convert.ToString(Box_BDR_parameters[i, 1]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "alpha 1," + Convert.ToString(Box_BDR_parameters[i, 2]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "alpha 2," + Convert.ToString(Box_BDR_parameters[i, 3]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "beta 1," + Convert.ToString(Box_BDR_parameters[i, 4]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "beta 2," + Convert.ToString(Box_BDR_parameters[i, 5]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "scale," + Convert.ToString(Box_BDR_parameters[i, 6]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "theta," + Convert.ToString(Box_BDR_parameters[i, 7]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "DMax," + Convert.ToString(Box_BDR_parameters[i, 8]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "D," + Convert.ToString(Box_BDR_parameters[i, 9]));
                                outfile2.WriteLine(saveName + "," + landmarkNames[i] + "," + "DI," + Convert.ToString(Box_BDR_parameters[i, 10]));
                            }
                            else
                            {
